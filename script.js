...
function initLegend() {
    ...
    // Define fields in the exact order shown in the image
    const fieldsToShow = [
    ...
    positionLegend.innerHTML = `
        ...
        <div class="legend-item">
            <div class="legend-label legend-sublabel">from left to right, oldest to newest</div>
        </div>
    `;
    ...
    sizeLegend.innerHTML = `
        <h3>Circle size</h3>
        <div class="legend-item">
            <div class="legend-label">number of pages</div>
        </div>
        <div class="legend-item">
            <div class="legend-label legend-sublabel">dedicated in the</div>
        </div>
        <div class="legend-item">
            <div class="legend-label legend-sublabel">book</div>
        </div>
    `;
    
    // Number in circles legend
    ...
    // Display circle examples with numbers 1-3
    for (let i = 1; i <= 3; i++) {
        const item = document.createElement('div');
...
function loadDataAndCreateVisualization() {
    ...
    const pathsToTry = [
        "data/geniuses_dataset_updated.csv",
        "geniuses_dataset_updated.csv",
        "data/geniuses_dataset.csv",
        "/data/geniuses_dataset_updated.csv",
        "../data/geniuses_dataset_updated.csv"
    ];
    ...
    sephirotStructure.forEach((sephirot) => {
        ...
        if (geniuses.length > 0) {
            ...
            geniuses.forEach((genius, i) => {
                ...
                nameItem.style.fontSize = '10px';
                nameItem.style.marginBottom = '4px';
                nameItem.style.color = '#333333';
                nameItem.style.lineHeight = '1.3';
...
function drawSephirotStructure(svg, width, height) {
    ...
    svg.selectAll(".sephirot-label")
        ...
        .attr("y", (d, i) => {
            ...
            if (i === 0) return d.y * height - 15; // Keter - higher
            if (i === 5) return d.y * height + 15; // Tiferet - lower
            if (i === 9) return d.y * height + 15; // Malkut - lower
            return d.y * height - 12;
        })
        .text(d => d.name);
}
